...
const Views = {
  ...
  mobileAdminDashboard() {
    ...
    const requestCards = State.requests.slice(0, 3).map(request => {
      const isReturnRequest = request.purpose === 'Return';
      const isPending = String(request.status || '').toLowerCase() === 'pending';
      const statusClass = request.status === 'approved' ? 'bg-emerald-100 text-emerald-800' : request.status === 'rejected' ? 'bg-error-container text-error' : 'bg-amber-100 text-amber-800';
      const actions = isPending ? `<div class="flex gap-xs"><button onclick="event.stopPropagation(); ${isReturnRequest ? `Actions.confirmReturn('${request.id}')` : `Actions.approveRequest('${request.id}')`}" class="bg-emerald-600 text-white px-2 py-1 rounded text-[10px] font-bold">${isReturnRequest ? 'Confirm' : 'Approve'}</button><button onclick="event.stopPropagation(); ${isReturnRequest ? `Actions.rejectReturn('${request.id}')` : `Actions.rejectRequest('${request.id}')`}" class="bg-error text-white px-2 py-1 rounded text-[10px] font-bold">Reject</button></div>` : '';
      return `<div onclick="Router.go('admin-requests')" class="flex items-center justify-between gap-sm border-b border-outline-variant py-sm last:border-0 cursor-pointer hover:bg-surface-container-low"><div class="min-w-0"><div class="font-medium text-sm truncate">${h(request.items?.name || 'Equipment')}</div><div class="text-xs text-on-surface-variant truncate">${h(request.profiles?.full_name || 'Member')} · ${fmtDate(request.requested_at)} · ${request.quantity || 1} unit(s)</div></div><div class="flex shrink-0 items-center gap-xs"><span class="px-2 py-1 rounded text-[10px] font-bold uppercase ${statusClass}">${h(request.status || (isReturnRequest ? 'Return' : 'Borrow'))}</span>${actions}</div></div>`;
    }).join('');
    const borrowCards = data.activeLoans.slice(0, 3).map(loan => {
      const isOverdue = new Date(loan.due_date) < new Date();
      return `<div onclick="Router.go('admin-loans')" class="flex items-center justify-between gap-sm border-b border-outline-variant py-sm last:border-0 cursor-pointer hover:bg-surface-container-low"><div class="min-w-0"><div class="font-medium text-sm truncate">${h(loan.items?.name || 'Equipment')}</div><div class="text-xs text-on-surface-variant truncate">${h(loan.profiles?.full_name || 'Member')} · Due ${fmtDate(loan.due_date)}</div></div><span class="px-2 py-1 rounded text-[10px] font-bold uppercase shrink-0 ${isOverdue ? 'bg-error-container text-error' : 'bg-emerald-100 text-emerald-800'}">${isOverdue ? 'Overdue' : 'Active'}</span></div>`;
    }).join('');
    ...
    return `
      <div class="space-y-md">
        ...
        <div class="grid grid-cols-1 gap-md"><section class="flex min-h-[280px] flex-col bg-surface-container-lowest rounded-xl border border-outline-variant p-md"><div class="flex items-center justify-between mb-sm"><div><h2 class="font-bold text-lg text-primary">Requests</h2><p class="text-xs text-on-surface-variant">Recent request status at a glance.</p></div><span class="text-sm font-bold text-amber-600">${data.pendingRequests.length}</span></div><div class="flex-1 space-y-sm">${requestCards || '<div class="text-sm text-on-surface-variant">No requests submitted.</div>'}</div><button onclick="Router.go('admin-requests')" class="w-full mt-md pt-sm border-t border-outline-variant text-secondary text-xs font-bold">See full list</button></section><section class="flex min-h-[280px] flex-col bg-surface-container-lowest rounded-xl border border-outline-variant p-md"><div class="flex items-center justify-between mb-sm"><div><h2 class="font-bold text-lg text-primary">Active Borrows</h2><p class="text-xs text-on-surface-variant">Current loans and due-date status.</p></div><span class="text-sm font-bold text-secondary">${data.activeLoans.length}</span></div><div class="flex-1 space-y-sm">${borrowCards || '<div class="text-sm text-on-surface-variant">No active borrows.</div>'}</div><button onclick="Router.go('admin-loans')" class="w-full mt-md pt-sm border-t border-outline-variant text-secondary text-xs font-bold">See full list</button></section></div>
  ...
  adminDashboard() {
    ...
    const requestCards = State.requests.slice(0, 4).map(request => {
      const isReturnRequest = request.purpose === 'Return';
      const isPending = String(request.status || '').toLowerCase() === 'pending';
      const statusClass = request.status === 'approved' ? 'bg-emerald-100 text-emerald-800' : request.status === 'rejected' ? 'bg-error-container text-error' : 'bg-amber-100 text-amber-800';
      const actions = isPending ? `<button onclick="event.stopPropagation(); ${isReturnRequest ? `Actions.confirmReturn('${request.id}')` : `Actions.approveRequest('${request.id}')`}" class="bg-emerald-600 text-white px-sm py-xs rounded text-xs font-bold">${isReturnRequest ? 'Confirm Return' : 'Approve'}</button><button onclick="event.stopPropagation(); ${isReturnRequest ? `Actions.rejectReturn('${request.id}')` : `Actions.rejectRequest('${request.id}')`}" class="bg-error text-white px-sm py-xs rounded text-xs font-bold">Reject</button>` : '';
      return `<div class="flex items-center justify-between gap-md border-b border-outline-variant py-sm last:border-0 cursor-pointer hover:bg-surface-container-low" onclick="Router.go('admin-requests')"><div class="min-w-0"><div class="font-medium text-sm">${h(request.items?.name || 'Equipment')}</div><div class="text-xs text-on-surface-variant">${h(request.profiles?.full_name || 'Member')} · ${fmtDate(request.requested_at)} · ${request.quantity || 1} unit(s)</div></div><div class="flex shrink-0 items-center gap-sm"><span class="px-2 py-1 rounded text-xs font-bold uppercase ${statusClass}">${h(request.status || (isReturnRequest ? 'Return' : 'Borrow'))}</span>${actions}</div></div>`;
    }).join('');
    const borrowCards = data.activeLoans.slice(0, 4).map(loan => {
      const isOverdue = new Date(loan.due_date) < new Date();
      return `<div class="flex items-center justify-between gap-md border-b border-outline-variant py-sm last:border-0 cursor-pointer hover:bg-surface-container-low" onclick="Router.go('admin-loans')"><div class="min-w-0"><div class="font-medium text-sm">${h(loan.items?.name || 'Equipment')}</div><div class="text-xs text-on-surface-variant">${h(loan.profiles?.full_name || 'Member')} · Due ${fmtDate(loan.due_date)}</div></div><span class="px-2 py-1 rounded text-xs font-bold uppercase shrink-0 ${isOverdue ? 'bg-error-container text-error' : 'bg-emerald-100 text-emerald-800'}">${isOverdue ? 'Overdue' : 'Active'}</span></div>`;
    }).join('');
    ...
    return `
      <div class="space-y-lg max-w-7xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-lg"><section class="flex min-h-[320px] flex-col bg-surface-container-lowest rounded-xl border border-outline-variant p-lg"><div class="flex items-center justify-between mb-sm"><div><h2 class="font-bold text-lg text-primary">Requests</h2><p class="text-xs text-on-surface-variant">Approve or reject pending requests quickly.</p></div><span class="text-sm font-bold text-amber-600">${data.pendingRequests.length}</span></div><div class="flex-1 space-y-sm">${requestCards || '<div class="text-sm text-on-surface-variant">No pending requests.</div>'}</div><button onclick="Router.go('admin-requests')" class="w-full mt-md pt-sm border-t border-outline-variant text-secondary text-xs font-bold">See full list</button></section><section class="flex min-h-[320px] flex-col bg-surface-container-lowest rounded-xl border border-outline-variant p-lg"><div class="flex items-center justify-between mb-sm"><div><h2 class="font-bold text-lg text-primary">Active Borrows</h2><p class="text-xs text-on-surface-variant">Monitor equipment currently checked out.</p></div><span class="text-sm font-bold text-secondary">${data.activeLoans.length}</span></div><div class="flex-1 space-y-sm">${borrowCards || '<div class="text-sm text-on-surface-variant">No active borrows.</div>'}</div><button onclick="Router.go('admin-loans')" class="w-full mt-md pt-sm border-t border-outline-variant text-secondary text-xs font-bold">See full list</button></section></div>
      </div>
    `;
  },
